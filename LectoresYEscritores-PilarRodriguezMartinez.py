...
#Inicializamos las variables globales que se utilizaran en el programa 
# k = Número total de iteraciones
# lec_espera = Número de lectores en espera
# es_espera = Número de escritores en espera
...
def esc(id,condition,lista,es_espera,lec_espera,queue):
  """
    Función para definir a los Escritores
  """
  for v in range(k):
  ...
  lista[0] += 1                 #Actualiza numero escritores
  es_espera -= 1                #Actualiza numero escritores en espera

  condition.release()	          #Suelta la condicion
  queue.put((id,"escribe",v))   #Actualiza la cola					
...
def lec(id,condition,lista,es_espera,lec_espera,queue):
  """
    Función para definir a los Lectores
  """
  ...
  lista[1] += 1                   #Actualiza el numero de lectores
  lec_espera -= 1                 #Actualiza el numero de lectores en espera
  
  condition.release()             #Suelta la condicion 
  queue.put((id,"lee",v))	        #Actualiza la cola        
  print "Lector lee",v 
  ...
  queue.put((id,"reflexiona",v))	  #Actualiza la cola  
